...
- 通过lua脚本，键入`ja`后可以输入日语假名，并可在固定的前三个假名候选后补充少量日文汉字。

`ja`模式的日文汉字映射集中写在`lua/jp_kanji_map.lua`中。例如键入`jashizuku`时，前三位依次为`しずく`、`シズク`、`ｼｽﾞｸ`，第四位为`雫`。这个映射表只用于补充中文输入和拆字反查都不方便的字符，不作为完整的日语词典使用。
